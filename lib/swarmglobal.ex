defmodule SwarmGlobal do
  ...
  for multinode environment.

  Based on [https://github.com/discordapp/fastglobal](FastGlobal).
  """

  @type t :: {__MODULE__, atom}

  @doc """
  Create a module for the SwarmGlobal instance.
  """
  ...
  @doc """
  Store `value` at `key`, replaces an existing value if present across all connected nodes.
  """
  @spec swarm_put(atom | {__MODULE__, module}, any) :: :ok
  def swarm_put({__MODULE__, module}, value), do: do_swarm_put(module, value)
  def swarm_put(key, value), do: key |> key_to_module |> do_swarm_put(value)
  ...
  defp do_swarm_put(module, value) do
    binary = compile(module, value)
    [Node.self() | Node.list()]
    |> Enum.each(fn node ->
      Node.spawn(node, fn ->
